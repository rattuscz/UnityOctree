...
public class BoundsQuadtree<T> {
	...
	void Grow(Vector3 direction) {
		...
		Vector3 newCenter = rootNode.Center + new Vector3(xDirection * half, yDirection * half, 0);

		// Create a new, bigger octree root node
		rootNode = new BoundsQuadtreeNode<T>(newLength, minSize, looseness, newCenter);

		// Create 7 new octree children to go with the old root as children of the new root
		int rootPos = GetRootPosIndex(xDirection, yDirection, 0);
		...
		for (int i = 0; i < 4; i++) {
			if (i == rootPos) {
				...
				yDirection = i > 1 ? -1 : 1;
				children[i] = new BoundsQuadtreeNode<T>(rootNode.BaseLength, minSize, looseness, newCenter + new Vector3(xDirection * half, yDirection * half, 0));
			}
		}
