...
public class BoundsQuadtreeNode<T> {
	...
	float looseness;
	// Minimum size for a node in this Quadtree
	float minSize;
	...
	readonly List<QuadtreeObject> objects = new List<QuadtreeObject>();
	// Child nodes, if any
	...
	/// <param name="minSizeVal">Minimum size of nodes in this Quadtree.</param>
	...
	/// <summary>
	/// Set the children of this quadtree.
	/// </summary>
	/// <param name="childQuadtrees">The 4 new child nodes.</param>
	public void SetChildren(BoundsQuadtreeNode<T>[] childQuadtrees) {
		if (childQuadtrees.Length != 4) {
			Debug.LogError("Child quadtree array must be length 4. Was length: " + childQuadtrees.Length);
			return;
		}
	...
	public void DrawAllObjects() {
		...
		foreach (QuadtreeObject obj in objects) {
	...
	/// <summary>
	/// We can shrink the quadtree if:
	/// - This node is >= double minLength in length
	/// - All objects in the root node are within one octant
	/// - This node doesn't have children, or does but 7/8 children are empty
	/// We can also shrink it if there are no objects left at all!
	/// </summary>
	/// <param name="minLength">Minimum dimensions of a node in this quadtree.</param>
	...
	public BoundsQuadtreeNode<T> ShrinkIfPossible(float minLength) {
		...
		int bestFit = -1;
		for (int i = 0; i < objects.Count; i++) {
			QuadtreeObject curObj = objects[i];
	...
	*/
	...
	void SubAdd(T obj, Bounds objBounds) {
		...
		if (objects.Count < numObjectsAllowed || (BaseLength / 2) < minSize) {
			QuadtreeObject newObj = new QuadtreeObject { Obj = obj, Bounds = objBounds };
			...
			if (children == null) {
				...
				for (int i = objects.Count - 1; i >= 0; i--) {
					QuadtreeObject existingObj = objects[i];
					// Find which child the object is closest to based on where the
					// object's center is located in relation to the quadtree's center.
	...
	/// <summary>
	/// Splits the quadtree into eight children.
	/// </summary>
	void Split() {
		float quarter = BaseLength / 4f;
		float newLength = BaseLength / 2;
		children = new BoundsQuadtreeNode<T>[4];
		children[0] = new BoundsQuadtreeNode<T>(newLength, minSize, looseness, Center + new Vector3(-quarter, quarter, 0));
		children[1] = new BoundsQuadtreeNode<T>(newLength, minSize, looseness, Center + new Vector3(quarter, quarter, 0));
		children[2] = new BoundsQuadtreeNode<T>(newLength, minSize, looseness, Center + new Vector3(-quarter, -quarter, 0));
		children[3] = new BoundsQuadtreeNode<T>(newLength, minSize, looseness, Center + new Vector3(quarter, -quarter, 0));
		/*
		children[4] = new BoundsQuadtreeNode<T>(newLength, minSize, looseness, Center + new Vector3(-quarter, -quarter, -quarter));
		children[5] = new BoundsQuadtreeNode<T>(newLength, minSize, looseness, Center + new Vector3(quarter, -quarter, -quarter));
		children[6] = new BoundsQuadtreeNode<T>(newLength, minSize, looseness, Center + new Vector3(-quarter, -quarter, quarter));
		children[7] = new BoundsQuadtreeNode<T>(newLength, minSize, looseness, Center + new Vector3(quarter, -quarter, quarter));
		*/
	}
	...
	void Merge() {
		...
		for (int i = 0; i < 4; i++) {
			...
			for (int j = numObjects - 1; j >= 0; j--) {
				QuadtreeObject curObj = curChild.objects[j];
				objects.Add(curObj);
			}
		}
	...
	int BestFitChild(Bounds objBounds) {
		return (objBounds.center.x <= Center.x ? 0 : 1) + (objBounds.center.y >= Center.y ? 0 : 2);
	}
